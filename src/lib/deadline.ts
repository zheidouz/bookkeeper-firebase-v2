// src/lib/deadline.ts
//
// Pure-function deadline calculator for Philippine BIR form filing
// dates. No side effects, no Firebase imports — safe to use in both the
// web client and Cloud Functions (the latter keeps a mirrored copy at
// functions/src/lib/deadline.ts; see docs/issues/0007-deadline-calculator.md).
//
// The function operates in UTC throughout so the result is reproducible
// across machines in any timezone. Callers may pre-convert local dates
// before calling.
 ...
 *
 * Helper used by slide logic and exposed for tests; not part of the
 * public API but harmless to use.
 */
...
function slideToWorkingDay(
  ...
  // Pathological: an entire two-week stretch of holidays/weekends. Return
  // whatever we ended up on rather than throw — callers should treat the
  // return as a deadline, not a guarantee.
  return cur;
}
...
export function calculateNextDeadline(
  ...
  // formCode is intentionally accepted-and-ignored. Reference it so
  // bundlers / linters do not flag it as unused and so future per-form
  // overrides have a hook.
  void formCode;
  ...
  switch (rule) {
    ...
    case "fixedDayOfMonthAfterPeriod": {
      ...
      // If `day` overflows the month (e.g. shift=31 in February), JS Date
      // rolls forward into the next month. That is fine — the slide logic
      // will still land on a working day. We accept it rather than masking.
      return slideToWorkingDay(base, 1, 0, holidays);
    }
    default: {
      // Exhaustiveness check. If a new rule is added to the union above,
      // TypeScript will fail this branch at compile time.
